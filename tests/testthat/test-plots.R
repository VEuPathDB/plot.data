...
test_that("histogram() returns an appropriately sized data.table", {
  ...
  viewport <- list('min'=min(bigData$var), 'max'=max(bigData$var))
  binReportValue <- 'binWidth'

  dt <- histogram.dt(df, map, binWidth = NULL, value='count', binReportValue, viewport)[[1]]
  expect_is(dt, 'data.table')
  expect_equal(nrow(dt),16)
  expect_equal(names(dt),c('group', 'panel', 'binLabel', 'binStart', 'value'))

  dt <- histogram.dt(df, map, binWidth=NULL, value='proportion', binReportValue, viewport)[[1]]
  expect_is(dt, 'data.table')
  expect_equal(nrow(dt),16)
  expect_equal(names(dt),c('group', 'panel', 'binLabel', 'binStart', 'value'))
