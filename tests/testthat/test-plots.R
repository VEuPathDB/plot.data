...
test_that("histogram() returns an appropriately sized data.table", {
  ...
  dt <- histogram.dt(df, map, binWidth=.1, value='count')
  expect_is(dt, 'data.table')
  expect_equal(nrow(dt),16)
  expect_equal(names(dt),c('panel', 'group', 'label', 'x', 'y'))

  dt <- histogram.dt(df, map, binWidth=.1, value='proportion')
  expect_is(dt, 'data.table')
  expect_equal(nrow(dt),16)
  expect_equal(names(dt),c('panel', 'group', 'label', 'x', 'y'))
