...
test_that("groupSmoothedMean() returns an appropriately sized data.table", {
  ...
  expect_is(dt, 'data.table')
  expect_equal(nrow(dt),1)
  expect_equal(names(dt), c('independent', 'y', 'ymin', 'ymax', 'se'))

  dt <- groupSmoothedMean(data.xy, 'x', 'y', 'group')
  expect_is(dt, 'data.table')
  expect_equal(nrow(dt),4)
  expect_equal(names(dt), c('independent', 'y', 'ymin', 'ymax', 'se', 'group'))

  dt <- groupSmoothedMean(data.xy, 'x', 'y', NULL, 'panel')
  expect_is(dt, 'data.table')
  expect_equal(nrow(dt),4)
  expect_equal(names(dt), c('independent', 'y', 'ymin', 'ymax', 'se', 'panel'))

  dt <- groupSmoothedMean(data.xy, 'x', 'y', 'group', 'panel')
  expect_is(dt, 'data.table')
  expect_equal(nrow(dt),16)
  expect_equal(names(dt), c('independent', 'y', 'ymin', 'ymax', 'se', 'group', 'panel'))
})
...
test_that("groupDensity() returns an appropriately sized data.table", {
  ...
  expect_is(dt, 'data.table')
  expect_equal(nrow(dt),1)
  expect_equal(names(dt), c('independent', 'y'))

  dt <- groupDensity(data.xy, 'y', 'group')
  expect_is(dt, 'data.table')
  expect_equal(nrow(dt),4)
  expect_equal(names(dt), c('group', 'independent', 'y'))

  dt <- groupDensity(data.xy, 'y', NULL, 'panel')
  expect_is(dt, 'data.table')
  expect_equal(nrow(dt),4)
  expect_equal(names(dt), c('panel', 'independent', 'y'))

  dt <- groupDensity(data.xy, 'y', 'group', 'panel')
  expect_is(dt, 'data.table')
  expect_equal(nrow(dt),16)
  expect_equal(names(dt), c('group', 'panel', 'independent', 'y'))
})
