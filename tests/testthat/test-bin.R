...
test_that("binProportion() returns an appropriately sized data.table", {
  viewport <- list('min'=min(data.xy$y), 'max'=max(data.xy$y)) 
  ...
  dt <- binProportion(data.xy, 'y','group', binWidth=.1, viewport=viewport)
  expect_is(dt, 'data.table')
  expect_equal(nrow(dt),4)
  expect_equal(names(dt), c('group', 'binLabel', 'binStart', 'value'))

  dt <- binProportion(data.xy, 'y', NULL, 'panel', binWidth=.1, viewport=viewport)
  expect_is(dt, 'data.table')
  expect_equal(nrow(dt),4)
  expect_equal(names(dt), c('panel', 'binLabel', 'binStart', 'value'))

  dt <- binProportion(data.xy, 'y', 'group', 'panel', binWidth=.1, viewport=viewport)
  expect_is(dt, 'data.table')
