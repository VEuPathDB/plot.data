...
test_that("box.dt() returns an appropriately sized data.table", {
  ...
  map <- data.frame('id' = c('group', 'y', 'panel'), 'plotRef' = c('overlayVariable', 'dependentVar', 'independentVar'), 'dataType' = c('STRING', 'NUMBER', 'STRING'), stringsAsFactors=FALSE)
  df <- data.xy
  ...
  map <- data.frame('id' = c('y', 'panel'), 'plotRef' = c('dependentVar', 'independentVar'), 'dataType' = c('NUMBER', 'STRING'), stringsAsFactors=FALSE)
